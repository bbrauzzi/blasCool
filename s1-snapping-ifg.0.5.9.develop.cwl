...
- class: Workflow
  ...
  inputs:
    ...
    datapipeline: 
      type: string
      label: datapipeline (Terradue user personal index)
      doc: datapipeline (Terradue user personal index)
    s3_bucket:
      type: string
      label: S3 bucket (provided without protocol)
      doc: S3 bucket (provided without protocol)
  outputs: 
...
- class: CommandLineTool
  ...
  requirements: 
    DockerRequirement:
      dockerPull: docker.terradue.com/s1-snapping-ifg:0.5.9-develop
    ResourceRequirement:
      coresMax: 2
      ramMax: 8000
    InlineJavascriptRequirement: {}
  ...
  - valueFrom: ${ return ["--datapipeline", inputs.datapipeline]; }
  - valueFrom: ${ return ["--first-run", inputs.first_run]; }
  ...
  - valueFrom: ${ if (inputs.s3_bucket == null){
                  return ["--empty"]
                  } else {
                  return ["--bucket", inputs.s3_bucket];
                  }
                  }
  inputs: 
    reference: 
    ...
    s3_bucket:
      type: string
      default: "cpe-e2e-runs"
  outputs:
    s3_catalog_output:
      outputBinding:
        outputEval: ${ return "s3://" + inputs.s3_bucket + "/" + inputs.datapipeline + "/" + inputs.stack_uuid + "/catalog.json"; }
      type: string
