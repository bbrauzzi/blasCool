...
- baseCommand: mosaic
  ...
  - valueFrom: ${ if (inputs.layer == "ESA World Cover") { return ["--esa-world-cover"]; }
                  ...
                  if (inputs.layer == "JRC Global Surface Water - Recurrence") { return ["--global-surface-water", "--layer", "recurrence"]}
                  if (inputs.layer == "JRC Global Surface Water - Seasonality") { return ["--global-surface-water", "--layer", "seasonality"]}
                  if (inputs.layer == "JRC Global Surface Water - Transitions") { return ["--global-surface-water", "--layer", "transitions"]}
                  if (inputs.layer == "DLR World Settlement Footprint") { return ["--world-settlement-footprint"]; }
                  if (inputs.layer == "World Population") { return ["--world-population"]; }
                }
  outputs:
    results:
      outputBinding:
